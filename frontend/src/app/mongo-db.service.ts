...
// 2024-07-24
  saveDemo(oid: string,sponsor_code:string, dem: Demographic): Observable<any> {
    ...
    if(this.useRealm){
      ...
      /*
      return this.http.post<User>(`${this.API}/demo`, query).subscribe(
...
*/
  ...
  getAllProxyPeople(): Observable<ProxyUser[]> {
    // return this.http.get<ProxyUser[]>(`${this.API}/proxyusers`).catchError(err: HttpErrorResponse) =>{return Observable.throwError(err)};
